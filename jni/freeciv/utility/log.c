...
#define LOG(lvl, ...) __android_log_print(lvl, "freecivlog", __VA_ARGS__);
...
#define LOG_DEBUG   4
	...
	switch (level) {
	case LOG_FATAL:   LOG(ANDROID_LOG_FATAL, message); break;
	case LOG_ERROR:   LOG(ANDROID_LOG_ERROR, message); break;
	case LOG_NORMAL:  LOG(ANDROID_LOG_DEFAULT, message); break;
	case LOG_VERBOSE: LOG(ANDROID_LOG_VERBOSE, message); break;
	case LOG_DEBUG:   LOG(ANDROID_LOG_DEBUG, message); break;
	}
